...
// Single onOpen() for the whole project — builds every menu. GAS shares one global
// namespace across .gs files, so there must be exactly ONE onOpen() (a second one in
// any file breaks the entire web app with "Script function not found"). The Email Agent
// menu (formerly its own onOpen in email_agent_drafts.gs) is merged in here.
function onOpen() {
