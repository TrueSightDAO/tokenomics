...
// - Format: "<botId>:<botToken>" (obtain from @BotFather). Never hardcode a real token.
...
function resolveRedirect(url) {
  try {
    ...
    try {
      const spreadsheet = SpreadsheetApp.openByUrl(OFFCHAIN_SHEET_URL);
...
function processNonAgl4Transactions() {
  ...
  for (let i = 1; i < sourceData.length; i++) {
    ...
    if (agroverseValue !== 'https://agroverse.shop/agl4' && agroverseValue && (!tokenizedStatus || tokenizedStatus === '')) {
      ...
      if (qrCode) {
        sendNonAgl4TransactionNotification(qrCode, cashContributorName, aglContractName, destSheetUrl);
      } else {
