/**
 * Credentials.js - 1wmgYPwf (TDG inventory mgmt:
 *   sales_update_main_dao_offchain_ledger + telegram_webhook_listener)
 ...
 *   sales_update_main_dao_offchain_ledger.js -> setApiKeys(); const creds = getCredentials();
 *   telegram_webhook_listener.js            -> setApiKeys(); const webhookCreds = getCredentials();
 *
 * Until 2026-09-18 this project had NO Credentials.js at all (neither tracked nor live),
 * so every entry point failed with `ReferenceError: setApiKeys is not defined`, including
 * the ANYONE_ANONYMOUS doPost Telegram webhook and the v5 sales-ledger web app.
 *
 * `clasp push` = projects.updateContent = REPLACES the project's remote file set: any file
 * NOT in the pushed set is DELETED from the live project. So this accessor is TRACKED (via a
 * .gitignore negation for this path only) and PUSHED like any other source - never
 * .claspignore'd. Secrets live in Script Properties (Project Settings > Script Properties),
 * which a push never touches.
 *
 * Usage in GAS code:
 *   setApiKeys();
 ...
 * can never overwrite a real live value. It exists because the entry-point files call it.
 */
function setApiKeys() {
