...
| `?action=listStoresByFilter` | Paginated Hit List rows: `status` and `shop_type` may be repeated (exact cell match). Optional **`warmup_bucket`** and **`followup_bucket`**: each an integer **0–7** (send-depth bucket for AU / AV: **0** sends, **1…6** exact counts, **7** = 7+ sends — same mapping as the Pipeline Dashboard). Omit both filters to return all rows (subject to limit). `limit` (default 200, max 500), `offset` (default 0). Response: `{ rows: [...], total, offset, limit, returned }` where each row includes `store_key`, `shop_name`, `status`, `shop_type`, `city`, `state`, `email`, `status_updated`, `hit_list_row`, and when AU/AV columns exist: **`warmup_sent`**, **`followup_sent`** (integer counts from sheet formulas). |
| `?action=listStatusSummary` | Pipeline-style counts from **Hit List** (one pass): `by_status[]` / `by_shop_type[]` with **`count`**, optional **`warmup`** / **`followup`** each an **array of eight integers** (store counts per depth bucket for columns **Warm-up email sent** / **Follow-up emails sent**), plus `total_data_rows`, `blank_status`, `blank_shop_type`, **`touch_metrics_available`**. Used by `stores_by_status.html`. |
