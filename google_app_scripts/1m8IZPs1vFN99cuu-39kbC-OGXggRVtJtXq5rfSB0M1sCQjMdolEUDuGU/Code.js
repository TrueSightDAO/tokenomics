...
function doGet(e) {
  ...
  if (action === 'processContributorAddsFromTelegramChatLogs') {
    // Edgar → GET ?action=processContributorAddsFromTelegramChatLogs
    // Implementation lives in ContributorAddHandler.js. Pulls pending
    // [CONTRIBUTOR ADD EVENT] rows from Telegram Chat Logs, extracts
    // name + email, appends to "Contributors contact information" on
    // the Main Ledger, logs to "New Contributor" dedup tab.
    return handleContributorAddRequest_({
      secret: e.parameter.secret,
      force: e.parameter.force || '',
    });
  }
