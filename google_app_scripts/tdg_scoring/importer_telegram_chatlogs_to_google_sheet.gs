    ...
    if (entry.message) {
      if (checkMessageRecordExists(entry.update_id)) {
        ...
        all_contributors.forEach(function(current_contributor_handle) {
          ...
          var contributionDetails = extractContributionDetails(contributor_text);
          if (contributionDetails && contributionDetails.attachedFilename && contributionDetails.destinationFileLocation) {
            var processedFileIds = [];
            file_ids.forEach(function(fileId, index) {
              if (!processedFileIds.includes(fileId)) {
                var destinationUrl = index === 0 ? contributionDetails.destinationFileLocation : generateUniqueGitHubFilename(contributionDetails.destinationFileLocation, index);
                if (destinationUrl && !checkFileExistsInGitHub(destinationUrl)) {
                  var uploaded = uploadFileToGitHub(fileId, destinationUrl, contributor_text);
                  if (uploaded) {
                    Logger.log(`Successfully uploaded file ${fileId} for contribution event to ${destinationUrl}`);
                  } else {
                    Logger.log(`Failed to upload file ${fileId} for contribution event to ${destinationUrl}`);
                  }
                } else if (!destinationUrl) {
                  Logger.log(`Failed to generate unique filename for file ID ${fileId}`);
                } else {
                  Logger.log(`File already exists at ${destinationUrl}, skipping upload`);
                }
...
// Function to extract contribution details from message
function extractContributionDetails(message) {
  // Normalize line endings and trim whitespace
  message = message.replace(/\r\n/g, '\n').trim();
  
  // Regex to match [CONTRIBUTION EVENT] format from report_dao_contribution.html
  ...
  if (!match) {
    Logger.log(`Regex failed to match contribution message: ${message}`);
    return null;
  }
