...
const STRIPE_CHECKOUT_SESSION_ID_COL = 2; // Column C (0-indexed): Stripe Session ID
const STRIPE_CHECKOUT_FEE_COL = 11; // Column L (0-indexed): Stripe Transaction Fee (dollars, written by Edgar at webhook time)
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      try {
        ...
        const sessionId = (row[STRIPE_CHECKOUT_SESSION_ID_COL] || '').toString().trim();
        ...
        // ISO-8601 date (yyyy-MM-dd) so the transparency explorer can parse it
        // via new Date(). yyyyMMdd is rejected as "Invalid Date" by JS Date.
        let dateStr;
        if (timestamp instanceof Date) {
          dateStr = Utilities.formatDate(timestamp, Session.getScriptTimeZone(), 'yyyy-MM-dd');
        } else {
          // Try to parse common audit-tab formats ('2026-05-10 21:14:35', '5/10/2026 ...') back into a Date.
        ...
        // Description leads with the Stripe session id (prominent for
        // at-a-glance dedup and viewer recognition — donors / auditors
        // can paste the cs_… ID directly into Stripe Dashboard). Col P
        // on the audit tab is the primary idempotency guard; the session id
        // in Description is a secondary scan-the-Transactions-tab guard so
        // a recovered/edited audit row can't double-write.
        const description = (sessionId ? sessionId + ' — ' : '')
          + itemsPurchased
          + (customerName ? ' — ' + customerName : '');

        // Dedup: scan the destination Transactions tab for the session id
        // before appending. Cheap full-column scan since ledgers have at most
        // hundreds of rows.
        if (sessionId) {
          const txValues = txSheet.getDataRange().getValues();
          const alreadyInTx = txValues.some(r => (r[1] || '').toString().indexOf(sessionId) !== -1);
          if (alreadyInTx) {
            Logger.log(`Session ${sessionId} already in ${ledgerId} Transactions; marking col P and skipping append`);
        ...
        // Entity = the fund handler that receives Stripe settlements and
        // disburses to the ledger's recipient. For TBM that's Gary Teh, who
        // forwards via Pix to Bico Duro. The Transactions tab has a data-
        // validation dropdown of known counterparties; FUND_HANDLER is already
        // a known entity used by other Stripe-sales routing in this script.
        const entity = FUND_HANDLER;

        // Write donation row: Date, Description, Entity, Amount, Currency, Type.
        // Type='Assets' matches the managed-ledger Sheet's accounting-category
        // convention (Assets / Liabilities / Equity / Income / Expense). The
        // Stripe-event sub-type lives in the Description via the embedded
        // Stripe session id (cs_test_… / cs_live_…), which downstream
        // consumers (e.g. tribomirimbahia.truesight.me explorer) use to
        // classify rows as Stripe-donation Inflow.
        ...
        // Stripe processing fee → second row (negative amount, Type=Assets).
        // Mirrors the Flow 3 SaaS-charge pattern in fetchStripeTransactions:
        // every gross inflow is paired with a fee outflow so the ledger's
        // net is amount + (-fee). Skip if fee column is blank or zero — that
        // just means Edgar hadn't resolved the BalanceTransaction yet at
        // webhook time; a future sweep can backfill the audit-tab cell and
        // re-trigger this routing to write the fee row.
        const feeValue = parseFloat(row[STRIPE_CHECKOUT_FEE_COL]);
        if (!isNaN(feeValue) && feeValue > 0) {
          const feeDescription = (sessionId ? sessionId + ' — ' : '')
            + 'Stripe processing fee'
            + (customerName ? ' — ' + customerName : '');
          const feeRowNum = findFirstEmptyRow(txSheet, 3);
          txSheet.getRange(feeRowNum, 1, 1, 6).setValues([[
            dateStr,
            feeDescription,
            entity,
            -feeValue,
          ...
          Logger.log(`  fee row: -$${feeValue.toFixed(2)} ${currency} → row ${feeRowNum}`);
        }
