...
/** Trusted agent contributor names (autopilot, Sophia) that can submit on behalf of governors. */
...
/**
 * Checks if a contributor name is a registered DAO governor.
 * Used for authorization bypass: governor-signed movements are always authorized.
 */
...
function inventoryMovementStatusFromTelegramRow_(telegramRow, contribution, warehouseManagerName) {
  ...
  // Governor-signed movements are always authorized (regardless of warehouse manager match)
  if (isGovernorByName_(res.contributorName)) return 'NEW';

  if (authNamesMatch_(res.contributorName, warehouseManagerName)) return 'NEW';

  // Agentic trust path: trusted agent (autopilot, Sophia) + governor-approved
