...
function triggerGitHubWebhook(sheetRow, qrCodeData) {
  try {
    ...
    // Prepare the repository_dispatch payload with full QR code data
    // so the workflow doesn't need to re-read the sheet
    var payload = {
      event_type: 'qr-code-generation',
      client_payload: {
        sheet_row: sheetRow.toString(),
        qr_code: qrCodeData.qr_code || '',
        product_name: qrCodeData.currency || '',
        landing_page: qrCodeData.landing_page || '',
        farm_name: qrCodeData.farm_name || '',
        state: qrCodeData.state || '',
        country: qrCodeData.country || '',
        year: qrCodeData.year || '',
...
function doGet(e) {
  try {
    ...
    if (actionStr === 'getTreeRecipientMap') {
      // DApp payout page: `tree_id` -> `pk_hash` so picking a tree auto-fills the
      // recipient. Hash-only, no raw PII. See process_payout_registration_telegram_logs.js
    ...
    if (actionStr === 'registerSingleQRCode') {
      ...
      // branded PNG generation. See register_single_qr_code.gs (same Apps Script project).
      return createCORSResponse(handleRegisterSingleQRCode(e.parameter));
    }
...
/**
 * Handle single QR code registration from Edgar webhook.
 * Called by doGet(?action=registerSingleQRCode).
 */
function handleRegisterSingleQRCode(params) {
  // --- Validate required fields ---
  var qrCode = params.qr_code ? String(params.qr_code).trim() : '';
  if (!qrCode) {
    return { status: 'error', message: 'Missing required parameter: qr_code' };
  }
  
  var landingPage = params.landing_page ? String(params.landing_page).trim() : '';
  if (!landingPage) {
    return { status: 'error', message: 'Missing required parameter: landing_page' };
  }
  
  var farmName = params.farm_name ? String(params.farm_name).trim() : '';
  if (!farmName) {
    return { status: 'error', message: 'Missing required parameter: farm_name' };
  }
  ...
  // --- Open sheet ---
  var spreadsheet = SpreadsheetApp.openByUrl('https://docs.google.com/spreadsheets/d/1GE7PUq-UT6x2rBN-Q2ksogbWpgyuh2SaxJyG_uEK6PU/edit?gid=472328231#gid=472328231');
  var sheet = spreadsheet.getSheetByName('Agroverse QR codes');
  if (!sheet) {
    return { status: 'error', message: 'Sheet not found: Agroverse QR codes' };
  }
  ...
  if (lastRow >= 2) {
    ...
    for (var i = 0; i < existingCodes.length; i++) {
      ...
      if (existing === qrCode) {
        return { status: 'error', message: 'QR code already exists: ' + qrCode, qr_code: qrCode, existing_row: i + 2 };
      }
    }
  }
  
  // --- Build row data (columns A-V) ---
  var today = creationDate || Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyyMMdd');
  var qrCodeLocation = 'https://github.com/TrueSightDAO/tokenomics/blob/main/generated_qr_codes/' + qrCode + '.png';
  
  var newRow = [
    qrCode,                    // A: qr_code
    landingPage,               // B: landing_page
    landingPage,               // C: ledger
    status,                    // D: status
    ...
    year,                      // H: Year
    currency,                  // I: Currency
    today,                     // J: QR code creation date
    qrCodeLocation,            // K: QR code location
    '',                        // L: Owner Email
  ...
  // Clear data validation on the target range to avoid dropdown/validation errors
  var targetRange = sheet.getRange(insertRow, 1, 1, newRow.length);
  ...
  // Trigger GitHub Actions webhook to generate branded QR PNG
  try {
    var qrCodeData = {
      qr_code: qrCode,
    ...
    var webhookResult = triggerGitHubWebhook(insertRow, qrCodeData);
    Logger.log('GitHub webhook: ' + webhookResult.message);
  } catch (webhookErr) {
    Logger.log('GitHub webhook failed (non-fatal): ' + webhookErr.message);
  }
